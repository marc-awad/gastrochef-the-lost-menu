...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.restaurant_name.length < 3) {
      setError('Le nom du restaurant doit contenir au moins 3 caractères');
      return;
    }
  ...
  const passwordStrength = () => {
    ...
    if (length < 6)
      return { label: 'Trop court', color: 'bg-red-500', width: 33 };
    if (length < 8) return { label: 'Moyen', color: 'bg-amber-500', width: 66 };
        ...
        <Card className="bg-white/80 backdrop-blur-sm shadow-2xl border-violet-200">
          <CardHeader className="bg-gradient-to-r from-violet-500 to-purple-600 text-white rounded-t-lg">
            ...
            <CardDescription className="text-violet-100">
              Lancez votre restaurant et découvrez les recettes perdues
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                {form.restaurant_name.length > 0 &&
                  form.restaurant_name.length < 3 && (
