...
const KpiCard = ({
  ...
  <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-5 backdrop-blur-sm">
    ...
    <p
      className={`text-2xl font-black tabular-nums ${positive === undefined ? 'text-white' : positive ? 'text-emerald-400' : 'text-rose-400'}`}
    >
      {value}
    </p>
...
// ─────────────────────────────────────────────
//  Tooltip personnalisé pour AreaChart
// ─────────────────────────────────────────────
...
export default function Dashboard() {
  ...
  // ── Chargement transactions (paginé + filtré) ────────────
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-8">
        ...
        {loading ? (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            {[...Array(4)].map((_, i) => (
              <div
                key={i}
                className="h-28 rounded-2xl bg-slate-800/40 animate-pulse"
              />
            ))}
          </div>
        ) : (
        ...
        {dashStats && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            {/* Area chart évolution trésorerie */}
        ...
        {/* ── Tableau rentabilité recettes ───────────────── */}
        {dashStats && dashStats.profitability.length > 0 && (
          <div className="mb-8 bg-slate-800/60 border border-slate-700/50 rounded-2xl overflow-hidden backdrop-blur-sm">
                ...
                <thead>
                  <tr className="border-b border-slate-700/50">
                    {[
                      ...
                      <th
                        key={h}
                        className="px-6 py-3 text-left text-xs text-slate-500 font-medium uppercase tracking-widest"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody>
                  {dashStats.profitability.map((row) => (
                    ...
                    >
                      ...
                      <td
                        className={`px-6 py-3 font-semibold tabular-nums ${row.margin >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                      >
                        {row.margin >= 0 ? '+' : ''}
                        {fmt(row.margin)}€
                      </td>
                ...
                {txLoading ? (
                  ...
                  transactions.map((tx) => (
                    <tr
                      ...
                      className="border-b border-slate-800/50 hover:bg-slate-700/20 transition-colors"
                    >
                      <td className="px-6 py-3 text-slate-400 text-xs tabular-nums whitespace-nowrap">
                      ...
                      <td
                        className={`px-6 py-3 font-semibold tabular-nums ${tx.amount >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                      >
                      ...
                      <td className="px-6 py-3 text-slate-300 tabular-nums font-medium">
